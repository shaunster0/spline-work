...
import numpy.lib.polynomial as ply #import *  # @UnusedWildImport
...
def polyint(p, m=1, k=None):
    ...
    truepoly = isinstance(p, ply.poly1d)
    p = np.asarray(p)
    if m == 0:
        if truepoly:
            return ply.poly1d(p)
...
def ortho2poly(p):
    ...
    for i in range(2, n + 1):
        x[i] = ply.polyadd(ply.polymul([1, - p[1, i]], x[i - 1]), - p[2, i] * x[i - 2])
    for i in range(n + 1):
        x[i] *= p[0, i]
    return reduce(ply.polyadd, x)
...
def polyshift(py, a=-1, b=1):
    ...
    >>> py = [1, 0]
    >>> px = polyshift(py,0,5)
    >>> ply.polyval(px,[0, 2.5, 5])  #% This is the same as the line below
    array([-1.,  0.,  1.])
    >>> ply.polyval(py,[-1, 0, 1 ])
    array([-1,  0,  1])
    """
...
def cheb2poly(ck, a=-1, b=1):
    ...
    for ix in range(n - 1):
        tmp = b_Nmi
        b_Nmi = ply.polymul(y2, b_Nmi)  # polynomial multiplication
        nb = len(b_Nmip1)
        b_Nmip1[-1] = b_Nmip1[-1] - ck[ix]
        b_Nmi[-nb::] = b_Nmi[-nb::] - b_Nmip1
        b_Nmip1 = tmp

    p = ply.polymul(y, b_Nmi)  # polynomial multiplication
    nb = len(b_Nmip1)
...
def chebpoly(n, x=None, kind=1):
    ...
    if x is None:  # Calculate coefficients.
        if n == 0:
            ...
            p = np.round(pow(2, n - 2 + kind) * ply.poly(chebroot(n, kind=kind)))
            p[1::2] = 0
        return p
...
class Cheb1d(object):
    ...
    def __init__(self, ck, a=-1, b=1, kind=1):
        if isinstance(ck, Cheb1d):
            ...
            return
        cki = ply.trim_zeros(np.atleast_1d(ck), 'b')
        if len(cki.shape) > 1:
    ...
    def __add__(self, other):
        other = Cheb1d(other)
        new = Cheb1d(self)
        new.coeffs = ply.polyadd(self.coeffs, other.coeffs)
        return new
    ...
    def __sub__(self, other):
        other = Cheb1d(other)
        new = Cheb1d(self)
        new.coeffs = ply.polysub(self.coeffs, other.coeffs)
        return new

    def __rsub__(self, other):
        other = Cheb1d(other)
        new = Cheb1d(self)
        new.coeffs = ply.polysub(other.coeffs, new.coeffs)
        return new
...
def padefit(c, m=None):
    ...
    >>> import matplotlib.pyplot as plt
    >>> c = ply.poly1d(1./sp.gamma(np.r_[6+1:0:-1]))
    >>> [p, q] = padefit(c)
    >>> p; q
...
def padefitlsq(fun, m, k, a=-1, b=1, trace=False, x=None, end_points=True):
    ...
    for ix in range(MAXIT):
        ...
        ee = ply.polyval(cof[m::-1], x) / \
            ply.polyval(cof[ncof:m:-1].tolist() + [1, ], x) - fs
    ...
    return ply.poly1d(c1), ply.poly1d(c2)
...
def main():
    ...
    x = np.linspace(0, 4)
    plt.plot(x, ply.polyval(c1, x) / ply.polyval(c2, x), 'g')
    plt.plot(x, exp(x), 'r')
    ...
    _t = ply.poly2hstr([1, 1, 2])
    py = [1, 0]
    px = polyshift(py, 0, 5)
    _t1 = ply.polyval(px, [0, 2.5, 5])  # % This is the same as the line below
    _t2 = ply.polyval(py, [-1, 0, 1])

    px = [1, 0]
    py = polyishift(px, 0, 5)
    t1 = ply.polyval(px, [0, 2.5, 5])  # % This is the same as the line below
    t2 = ply.polyval(py, [-1, 0, 1])
    print(t1, t2)
...
def test_polydeg():
    ...
    p0 = ortho2poly(p)
    p1 = ply.polyfit(x, ys0, n)
    plt.plot(xi, ply.polyval(p0, xi), 'g-.', xi, ply.polyval(p1, xi), 'go')
    plt.show('hold')
